...
export default function Screenshot({
  ...
  setImagesTaken: Dispatch<SetStateAction<boolean>>;
  setElementSSMapping: Dispatch<SetStateAction<any[]>>;
  background: any;
}) {
  ...
  const captureImage = () => {
    ...
    htmlToImage
      .toPng(document.getElementById("mappp")!)
      .then(function (dataUrl: any) {
        setImages((prevImages: any) => [
          ...prevImages,
          { id: imgUUID, url: dataUrl },
        ]);
      });
  };
  ...
  useEffect(() => {
    ...
    let interval = setInterval(() => {
      ...
      const newSectionTarget = Object.values(result.sections)[
        currentSectionIndex
      ];
      const maxRowIndex =
        Object.keys(newSectionTarget.rows).length - 1 >= 0
          ? Object.keys(newSectionTarget.rows).length - 1
          : 0;
      ...
      setSelectedIds({
        section: newSectionTarget.sectionId!,
        row: newTargetRow,
      });
  ...
  return (
    <div className='max-w-5xl flex flex-col gap-8 justify-center items-center'>
      <ActionsProvider>
        <SeatMap data={result} selectedIds={selectedIds} background={background} />
      </ActionsProvider>
