...
export const ActionsProvider = ({ children, data, setData, setHotspotSet }) => {
  ...
  const allSectionsSelected = () => {
    ...
    console.log("allSectionsSelected allSelectedSections.length: ", allSelectedSections.length)
  ...
  const isSelectContinueDisabled = (index) => {
    ...
    if (selectingIndex === floors - 1) {
      console.log("selectingOtherFloor || noSectionsInFloor || !allSectionsSelected();", selectingOtherFloor || noSectionsInFloor || !allSectionsSelected());
      return selectingOtherFloor || noSectionsInFloor || !allSectionsSelected();
    }

    console.log("selectingOtherFloor || noSectionsInFloor: ", selectingOtherFloor || noSectionsInFloor)
    ...
    >
      {activeMapAction === 4 && (
        <div className='flex flex-row items-center gap-3 justify-center w-full'>
          <button
            onClick={() => setActiveSpot("seats")}
            className={`${
              activeSpot === "seats"
                ? "bg-indigo-700 text-white"
                : "text-zinc-950 bg-white"
            } active:scale-95 rounded-lg px-5 py-2`}
          >
            Seats
          </button>
          <button
            onClick={() => setActiveSpot("rows")}
            className={`${
              activeSpot === "rows"
                ? "bg-indigo-700 text-white"
                : "text-zinc-950 bg-white"
          ...
          <button
            onClick={() => setActiveSpot("sections")}
            className={`${
              activeSpot === "sections"
                ? "bg-indigo-700 text-white"
                : "text-zinc-950 bg-white"
      ...
      {!allDone &&
        Array.from({ length: floors }, (_, i) => i + 1).map((floor, index) => {
          return (
            <div
              key={index}
              className={`${
            ...
            >
                  ...
                  {sectionsInFloor[index]
                    ? sectionsInFloor[index].map((section, sectionIndex) => (
                        <span
                          ...
                          style={{ backgroundColor: hex[index] }}
                          key={sectionIndex}
                        >
                          {section}
                        </span> // Assuming 'section' is a string or number
                      ))
                    : " None"}
                </div>
              </div>
              <button
                ...
                disabled={isSelectContinueDisabled(index)}
                className='px-5 py-2 disabled:bg-gray-300 bg-indigo-600 rounded-md text-white'
              >
                Ready?
              </button>
            </div>
        ...
        Array.from({ length: floors }, (_, i) => i + 1).map((floor, index) => {
          return (
            <div
              key={index}
              className={`${
                settingIndex === index && "ring-4 ring-indigo-600"
              } p-5 bg-white text-zinc-800 rounded-lg flex flex-row items-center justify-between`}
            >
              <h3 className='text-xl uppercase text-zinc-400 text-center font-bold tracking-[0.3em]'>
                PLACE HOTSPOT FOR FLOOR {index}
              </h3>
              <button
                onClick={continueSetting}
                disabled={isContinueSettingDisabled(index)}
                className='px-5 py-2 disabled:bg-gray-300 bg-indigo-600 rounded-md text-white'
              >
                Continue?
              </button>
      ...
      <div
        id='mapp'
        ref={mapRef}
        className={`cursor-pointer ${dragging && "indicate-drag"}`}
      >
        {children}
        {activeMapAction === 4 && (
          <DraggableSpot
